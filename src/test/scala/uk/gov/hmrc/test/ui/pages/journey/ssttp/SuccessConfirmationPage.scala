...
object SuccessConfirmationPage extends BasePage {
  ...
  def upfrontPaymentContent: String = cssSelector("#content > section > p:nth-child(2)").webElement.getText

    def assertContent(): Assertion =  {
      if (langToggle == Language.welsh) pageContent should be(WelshContent.successConfirmationContent())
      else pageContent should be(EnglishContent.successConfirmationContent())
    }
